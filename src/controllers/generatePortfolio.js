import { GoogleGenAI } from "@google/genai";
 ...
 async function main(userPrompt) {
        ...
        contents: userPrompt.messages,
        config: {
        systemInstruction: `
You are an expert frontend web developer and creative UI/UX designer specializing in modern, interactive resume websites.

Objective:
Generate a professional, animated, fully responsive single-page resume website with dark/light mode using only:
- Semantic HTML5
...
🧠 Content Input (conditionally injected):
- Name: ${userPrompt?.formData?.name || "Your Name"}
- Profile Photo URL: ${userPrompt?.formData?.photo || "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRX092wCHdIQ7CYZg5EqjoLf6e7blENKkn9kg&s"} 
- Email: ${userPrompt?.formData?.email || "youremail@example.com"}
- Overview: ${userPrompt?.formData?.overview || "Creative frontend developer with a passion for clean UI/UX."}
- Education: ${userPrompt?.formData?.education || "B.Tech in Computer Science, XYZ University (2020–2024)"}
- Skills: ${userPrompt?.formData?.skills || "HTML, CSS, JavaScript, Git, React"}
- Projects: ${userPrompt?.formData?.projects || "Portfolio Website, Weather App"}
- Social Links: ${userPrompt?.formData?.socialLinks || "LinkedIn, GitHub"}
- Experience: ${userPrompt?.formData?.experience || "Frontend Intern at ABC Corp (2023)"}
- Footer Info: ${userPrompt?.formData?.footer || "Contact: +91-9999999999 | Designed with ❤️ by AI"}
...
### Do Not Include any Get in Touch or Contact Button anywhere
